            ...
            stage('Sanity check') {
                steps {
                    emailext (
                        ...
                        body: "<html> <body> <h3>Cloning the Application from GitHub is succesful.<br> Do you want to start the building of the project? <br> <br> <form action=${env.BUILD_URL}input> <input type=submit value= ClickHere <br> ${env.COMMENT} = input ${env.message}: 'Start Building the Project.'</form></h2></body><html>",
                    attachLog: true,
                    )
                    script {
